...
void epdSetLut(void) {
    unsigned int count;     
    epdSendCommand(LUT_FOR_VCOM);                            //vcom
    for(count = 0; count < 36; count++) {
        epdSendData(lut_vcom0[count]);
    }
    
    epdSendCommand(LUT_WHITE_TO_WHITE);                      //ww --
    for(count = 0; count < 36; count++) {
        epdSendData(lut_ww[count]);
    }   
    
    epdSendCommand(LUT_BLACK_TO_WHITE);                      //bw r
    for(count = 0; count < 36; count++) {
        epdSendData(lut_bw[count]);
    } 

    epdSendCommand(LUT_WHITE_TO_BLACK);                      //wb w
    for(count = 0; count < 36; count++) {
        epdSendData(lut_bb[count]);
    } 

    epdSendCommand(LUT_BLACK_TO_BLACK);                      //bb b
    for(count = 0; count < 36; count++) {
        epdSendData(lut_wb[count]);
    } 
}
